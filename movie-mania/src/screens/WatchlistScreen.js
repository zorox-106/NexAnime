...
import { Text, FAB, Menu, Button } from 'react-native-paper';
...
export default function WatchlistScreen() {
    ...
    const { watchlist, getWatchlistMovies } = useWatchlist();
    ...
    const fetchWatchlistMovies = async () => {
        try {
            setLoading(true);
            // Fetch full anime details for watchlist items
            const animePromises = watchlist.map((id) => animeService.fetchAnimeDetails(id));
        ...
        <View style={[styles.container, { backgroundColor: theme.background }]}>
            ...
            {movies.length === 0 ? (
                <EmptyState
                    ...
                    message="Add movies to your watchlist to watch them later"
                />
            ) : (
                <>
                    {/* Sort Button */}
